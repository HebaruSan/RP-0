...
namespace RP0.ProceduralAvionics
{
    public partial class ModuleProceduralAvionics
    {
        ...
        private void WindowFunction(int windowID)
        {
            ...
            _selectedConfigIndex = GUILayout.Toolbar(_selectedConfigIndex, configNames, HighLogic.Skin.button);
            string curCfgName = configNames[_selectedConfigIndex];
        ...
        private void ApplyAvionicsSettings()
        {
            ...
            if (!float.TryParse(_sECAmount, out float ecAmount) || ecAmount <= 0)
            {
                ...
                _sECAmount = 100.ToString();
                return;
            }

            var rfpm = part.Modules.GetModule<ModuleFuelTanks>();
            ...
            var tankList = (FuelTankList)fiTanks.GetValue(rfpm);
            FuelTank ecTank = tankList["ElectricCharge"];

            controllableMass = newControlMass;
            if (_seekVolumeMethod != null)
            {
                SetProcPartVolumeLimit();
                ApplyCorrectProcTankVolume(extraVolumeLiters, ecAmount, rfpm, ecTank);
            }
            else
            {
        ...
        private void ApplyCorrectProcTankVolume(float extraVolumeLiters, float ecAmount, ModuleFuelTanks rfpm, FuelTank ecTank)
        {
            ...
            float utilizationPercent = (float)fiUtil.GetValue(rfpm);
            float utilization = utilizationPercent / 100;

            float avVolume = GetAvionicsVolume();
            float avUtil = InternalTanksAvailableVolumeUtilization;
            ...
            float lVolStep4 = ecAmount / ecTank.utilization;
            ...
            float m3TotalVolume = Math.Max((avUtil * avVolume + m3VolStep2) / avUtil, m3VolStep2 / avUtil);
